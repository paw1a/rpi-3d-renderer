...
struct sd_card_t {
    const char *pcName;
    spi_t *spi;
    // Slave select is here instead of in spi_t because multiple SDs can share
    // an SPI.
    uint ss_gpio; // Slave select for this SD card
    bool use_card_detect;
    uint card_detect_gpio;   // Card detect; ignored if !use_card_detect
    uint card_detected_true; // Varies with card socket; ignored if
                             // !use_card_detect
    ...
    int (*read_blocks)(sd_card_t *sd_card_p, uint8_t *buffer,
