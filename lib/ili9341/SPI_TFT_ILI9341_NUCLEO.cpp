...
SPI_TFT_ILI9341::SPI_TFT_ILI9341(PinName mosi, PinName miso, PinName sclk,
                                 PinName cs, PinName reset, PinName dc,
                                 const char *name)
    : GraphicsDisplay(name), SPI(mosi, miso, sclk, NC), _cs(cs), _reset(reset),
      _dc(dc) {
...
void SPI_TFT_ILI9341::f_write(int data) {
    ...
    case (1):
        while (bit_SPI1_txe == 0)
            ; // wait for SPI1->SR TXE flag
        SPI1_DR = data;
        break;

    case (2):
        while (bit_SPI2_txe == 0)
            ; // wait for SPI2->SR TXE flag
        SPI2_DR = data;
        break;

    case (3):
        while (bit_SPI3_txe == 0)
            ; // wait for SPI3->SR TXE flag
        SPI3_DR = data;
        break;
    }
}
...
void inline SPI_TFT_ILI9341::spi_bsy(void) {
    ...
    case (1):
        while (bit_SPI1_bsy == 1)
            ; // SPI1->SR bit 7
        break;

    case (2):
        while (bit_SPI2_bsy == 1)
            ; // SPI2->SR bit 7
        break;

    case (3):
        while (bit_SPI3_bsy == 1)
            ; // SPI2->SR bit 7
        break;
    }
}
...
void SPI_TFT_ILI9341::spi_16(bool s) {
    ...
    case (1):
        if (s)
            bit_SPI1_dff = 1; // switch to 16 bit Mode
        else
            bit_SPI1_dff = 0; // switch to 8  bit Mode
        break;

    case (2):
        if (s)
            bit_SPI2_dff = 1; // switch to 16 bit Mode
        else
            bit_SPI2_dff = 0; // switch to 8  bit Mode
        break;

    case (3):
        if (s)
            bit_SPI3_dff = 1; // switch to 16 bit Mode
        else
            bit_SPI3_dff = 0; // switch to 8  bit Mode
        break;
    }
}
...
void SPI_TFT_ILI9341::tft_reset() {
    ...
    case (1):
        RCC_AHBPeriphClockCmd(RCC_AHBPeriph_DMA1,
                              ENABLE); // SPI1 and SPI2 are using DMA 1
...
// optimized
// write direct to SPI1 register !
void SPI_TFT_ILI9341::window(unsigned int x, unsigned int y, unsigned int w,
                             unsigned int h) {
    wr_cmd(0x2A);
    spi_16(1);
    f_write(x);
...
void SPI_TFT_ILI9341::fillrect(int x0, int y0, int x1, int y1, int color) {
    ...
    case (3):
        ...
        do {
            ...
            while (DMA_GetCurrDataCounter(DMA2_Channel2) != 0)
                ; // wait for end of transfer
...
void SPI_TFT_ILI9341::Bitmap(unsigned int x, unsigned int y, unsigned int w,
...
// local filesystem is not implemented but you can add a SD card to a different
// SPI

int SPI_TFT_ILI9341::BMP_16(unsigned int x, unsigned int y,
                            const char *Name_BMP) {
...
#define OffsetBPP 28
    ...
    PixelHeigh = BMP_Header[OffsetPixelHeigh] +
                 (BMP_Header[OffsetPixelHeigh + 1] << 8) +
                 (BMP_Header[OffsetPixelHeigh + 2] << 16) +
                 (BMP_Header[OffsetPixelHeigh + 3] << 24);
    PixelWidth = BMP_Header[OffsetPixelWidth] +
                 (BMP_Header[OffsetPixelWidth + 1] << 8) +
                 (BMP_Header[OffsetPixelWidth + 2] << 16) +
                 (BMP_Header[OffsetPixelWidth + 3] << 24);
    if (PixelHeigh > height() + y || PixelWidth > width() + x) {
        fclose(Image);
        return (-3); // to big
    }

    start_data = BMP_Header[OffsetPixData] +
                 (BMP_Header[OffsetPixData + 1] << 8) +
                 (BMP_Header[OffsetPixData + 2] << 16) +
                 (BMP_Header[OffsetPixData + 3] << 24);

    line =
        (unsigned short *)malloc(2 * PixelWidth); // we need a buffer for a line
    if (line == NULL) {
