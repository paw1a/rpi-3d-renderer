...
SPI_TFT_ILI9341::SPI_TFT_ILI9341(PinName mosi, PinName miso, PinName sclk,
                                 ...
                                 const char *name)
    : GraphicsDisplay(name), SPI(mosi, miso, sclk), _cs(cs), _reset(reset),
      _dc(dc) {
...
void SPI_TFT_ILI9341::spi_16(bool s) {
    if (s)
        _spi.spi->CR0 |= 0x08; // switch to 16 bit Mode
    else
        _spi.spi->CR0 &= ~(0x08); // switch to 8  bit Mode
}
...
void SPI_TFT_ILI9341::fillrect(int x0, int y0, int x1, int y1, int color) {
    ...
    case (0):
        LPC_GPDMACH0->DMACCDestAddr =
            (uint32_t)&LPC_SSP0->DR; // we send to SSP0
        LPC_SSP0->DMACR = 0x2;
        break;
    case (1):
        LPC_GPDMACH0->DMACCDestAddr =
            (uint32_t)&LPC_SSP1->DR; // we send to SSP1
        LPC_SSP1->DMACR = 0x2;
        break;
    }
    ...
    do {
        ...
        LPC_GPDMACH0->DMACCControl =
            dma_count | (1UL << 18) | (1UL << 21) |
            (1UL << 31); // 16 bit transfer , no address increment, interrupt
        LPC_GPDMACH0->DMACCConfig =
            DMA_CHANNEL_ENABLE | DMA_TRANSFER_TYPE_M2P |
            (spi_num ? DMA_DEST_SSP1_TX : DMA_DEST_SSP0_TX);
...
void SPI_TFT_ILI9341::character(int x, int y, int c) {
    ...
    if (buffer != NULL) {                   // there is memory space -> use dma
        ...
        do {
            ...
            LPC_GPDMACH0->DMACCControl =
                dma_count | (1UL << 18) | (1UL << 21) | (1UL << 31) |
                DMA_CHANNEL_SRC_INC; // 16 bit transfer , address increment,
                                     // interrupt
            LPC_GPDMACH0->DMACCConfig =
                DMA_CHANNEL_ENABLE | DMA_TRANSFER_TYPE_M2P |
                (spi_num ? DMA_DEST_SSP1_TX : DMA_DEST_SSP0_TX);
            ...
            do {
            } while ((LPC_GPDMA->DMACRawIntTCStat & 0x01) ==
                     0); // DMA is running
